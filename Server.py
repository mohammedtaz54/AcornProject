...
def addContractorDetails():
    ...
    if request.method =='POST':
        ...
        try:
            ...
            #     cur = conn.cursor()
            #     cur.execute("INSERT INTO form_data (fieldlist[i])\
            #                 VALUES (?)", (valuelist[i]) )
            #     conn.commit()
            conn = sqlite3.connect(DATABASE)
            cur = conn.cursor()
            ...
            # conn = sqlite3.connect(DATABASE)
            # cur = conn.cursor()
            # cur.execute("INSERT INTO form_data ('proofOfEligibility', 'licence', 'criminalConviction', 'criminalDetails', 'disability','disabilityDetails', 'refereeName1', 'refereeJob1', 'refereeComp1', 'refereeAddress1','refereeNum1','refereeEmail1', 'refereeName2', 'refereeJob2', 'refereeComp2','refereeAddress2','refereeNum2','refereeEmail2')\
            #             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (valuelist[19],valuelist[20],valuelist[21],valuelist[22],valuelist[23],valuelist[24],valuelist[25],valuelist[26],valuelist[27],valuelist[28],valuelist[29],valuelist[30],valuelist[31],valuelist[32],valuelist[33],valuelist[34],valuelist[35],valuelist[36]) )
            # print("Second gucci")
            # conn.commit()
